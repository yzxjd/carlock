    ...
    function()
        ...
        while true do
            ...
            if socket.isReady() then
                ...
                --创建一个MQTT客户端
                local mqttClient = mqtt.client(imei,180,--,"lebo","MQTT-lebo123456")--,nil,
                -- local mqttClient = mqtt.client(imei,30,"xjdGZS","xjd123456",nil,--,"lebo","MQTT-lebo123456")--,nil,
                        --will 遗言
                        {qos=1, retain=1, topic="lebo/parklot/getstatus",payload="{\"VER\":\"0\",\"CMD\":\"12\",\"MD\":\"8\",\"CD\":\""..lockInit.LockDeviceID..
                ...
                if mqttClient:connect("112.74.132.1",1884,"tcp") then
                    ...
                    ready = true
                    --订阅主题
                    --if mqttClient:subscribe({["xjd"]=0, ["xjd/control"]=1, ["xjd/set"]=2}) then
                    if mqttClient:subscribe({["lebo/parklot/set/8"]=0, ["lebo/parklot/lock/8"]=1}) then
                        mqttOutMsg.init()
                        --循环处理接收和发送的数据
                        while true do
